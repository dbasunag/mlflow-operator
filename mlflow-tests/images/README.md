...
The same safe PostgreSQL/S3 rows enable `spec.garbageCollection` with the non-firing schedule `0 0 1 1 *`. Its smoke test soft-deletes an experiment containing a run artifact, creates a one-off Job from `mlflow-gc`, and verifies that run and experiment metadata plus the S3 object are permanently removed. It is also marked for the dedicated split artifact-server S3 matrix, exercising deletion of persisted `mlflow-artifacts:/` locations.
